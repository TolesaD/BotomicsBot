// wallet/app.js - COMPLETE PRODUCTION VERSION MATCHING YOUR BACKEND
...
async function refreshAllData() {
  ...
  try {
    ...
    if (balanceEl && !balanceEl.innerHTML.includes('Loading')) {
      balanceEl.innerHTML = `
        <div style="font-size: 36px; font-weight: bold; color: #999;">
          🔄 Loading...
        </div>
      `;
    }
...
// Add this function to track section changes
function setupAutoRefreshOnNavigation() {
  // Track when user returns to main wallet view
  const sectionObserver = new MutationObserver(function(mutations) {
    mutations.forEach(function(mutation) {
      if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
        const mainContainer = document.getElementById('mainContainer');
        if (mainContainer && !mainContainer.classList.contains('hidden')) {
          // Check if we're in the main wallet view (not a specific section)
          const isInMainView = !document.querySelector('.section:not(.hidden)');
          if (isInMainView && !isLoadingData) {
            // User returned to main view, refresh data
            setTimeout(async () => {
              await refreshAllData();
            }, 1000);
          }
...
async function loadBalance() {
  try {
    ...
    if (balanceEl) {
      balanceEl.innerHTML = `
        <div style="font-size: 36px; font-weight: bold; color: #999;">
          0.00 <span style="font-size: 24px;">BOM</span>
        </div>
        <div style="color: #666; margin-top: 5px;">$0.00 USD</div>
    ...
    if (statusEl) {
      statusEl.innerHTML = '<span style="color: #e74c3c;">⚠️ Tap refresh</span>';
    }
...
function updateBalanceUI(balance) {
  ...
  if (balanceEl) {
    balanceEl.innerHTML = `
      <div style="font-size: 36px; font-weight: bold; color: #4CAF50;">
        ${balance.balance.toFixed(2)} <span style="font-size: 24px;">BOM</span>
      </div>
      <div style="color: #666; margin-top: 5px;">$${balance.balance.toFixed(2)} USD</div>
    `;
  }
  ...
  if (statusEl) {
    if (balance.isFrozen) {
      statusEl.innerHTML = '<span style="color: #e74c3c;">❄️ Frozen</span>';
...
async function submitDepositProof() {
  ...
  // Note: File upload needs backend implementation
  // For now, we'll just submit without file
  try {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // FAQ toggle functionality - FIXED: Properly handle FAQ visibility
