// config/environment.js - RENDER DEPLOYMENT VERSION
...
function createConfig() {
  ...
  // Better Render detection
  const isRender =
    process.env.RENDER_EXTERNAL_URL ||
    process.env.RENDER_URL ||
    process.env.RENDER === "true";

  const isRailway = false; // Always false on Render
  ...
  if (isRender) {
    // Render production - prioritize RENDER_EXTERNAL_URL
    if (process.env.RENDER_EXTERNAL_URL) {
      const baseUrl = ensureHttps(process.env.RENDER_EXTERNAL_URL);
      appUrl = baseUrl;
      publicUrl = baseUrl;
      ...
      console.log(`🚀 Render: Using RENDER_EXTERNAL_URL: ${baseUrl}`);
    }
    // Fallback to RENDER_URL
    else if (process.env.RENDER_URL) {
      const baseUrl = ensureHttps(process.env.RENDER_URL);
      appUrl = baseUrl;
      publicUrl = baseUrl;
      ...
      console.log(`🚀 Render: Using RENDER_URL: ${baseUrl}`);
    }
  ...
  if (!appUrl && isProduction) {
    appUrl = "https://botomics.onrender.com";
    publicUrl = appUrl;
  ...
  return {
    ...
    ENABLE_PREMIUM_SUBSCRIPTIONS:
      process.env.ENABLE_PREMIUM_SUBSCRIPTIONS !== "false",
    BOTOMICS_PREMIUM_PRICE: parseFloat(process.env.BOTOMICS_PREMIUM_PRICE) || 5,
    BOTOMICS_MIN_WITHDRAWAL:
      parseFloat(process.env.BOTOMICS_MIN_WITHDRAWAL) || 20,

    // ==================== FEATURE FLAGS ====================
    ENABLE_ADVERTISING_ECOSYSTEM:
      process.env.ENABLE_ADVERTISING_ECOSYSTEM !== "false",
    ENABLE_MINI_BOT_DASHBOARD:
      process.env.ENABLE_MINI_BOT_DASHBOARD !== "false",
    ...
    MINI_BOT_COMMANDS_ENABLED:
      process.env.MINI_BOT_COMMANDS_ENABLED !== "false",
    ...
    ENABLE_OWNERSHIP_TRANSFER:
      process.env.ENABLE_OWNERSHIP_TRANSFER !== "false",
    ENABLE_PINNED_MESSAGES: process.env.ENABLE_PINNED_MESSAGES !== "false",

    // ==================== ADVERTISING SHARES ====================
    BOTOMICS_AD_PLATFORM_SHARE:
      parseFloat(process.env.BOTOMICS_AD_PLATFORM_SHARE) || 0.2,
    BOTOMICS_AD_BOT_OWNER_SHARE:
      parseFloat(process.env.BOTOMICS_AD_BOT_OWNER_SHARE) || 0.6,
    BOTOMICS_AD_USER_SHARE:
      parseFloat(process.env.BOTOMICS_AD_USER_SHARE) || 0.2,
  };
}
